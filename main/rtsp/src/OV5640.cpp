...
camera_config_t esp32cam_aithinker_config {
    ...
    .pin_href = 21,
    .pin_pclk = 14,
    .xclk_freq_hz = 24000000, // lower is probably more stable, but 8MHz proved very unstable
    .ledc_timer = LEDC_TIMER_0,
    .ledc_channel = LEDC_CHANNEL_0,
    .pixel_format = PIXFORMAT_JPEG,
    // .frame_size = FRAMESIZE_UXGA, // needs 234K of framebuffer space
...
void OV5640::done(void)
{
    DEBUG_PRINT("OV5640: done\n");
    if (fb)
    {
        DEBUG_PRINT("OV5640: done fb return\n");
...
void OV5640::run(void)
{
    DEBUG_PRINT("OV5640: run\n");
    if (fb)
    {
        DEBUG_PRINT("OV5640: run fb return\n");
        //return the frame buffer back to the driver for reuse
        esp_camera_fb_return(fb);
    }

    DEBUG_PRINT("OV5640: run fb get\n");
    fb = esp_camera_fb_get();
    DEBUG_PRINT("OV5640: run fb gotten\n");
}

void OV5640::runIfNeeded(void)
{
    if (!fb)
    {
        DEBUG_PRINT("OV5640: runIfNeeded running\n");
        run();
    }
}

int OV5640::getWidth(void)
{
    runIfNeeded();
    return fb->width;
}

int OV5640::getHeight(void)
{
    runIfNeeded();
    return fb->height;
}

size_t OV5640::getSize(void)
{
    runIfNeeded();
    if (!fb)
    {
...
framesize_t OV5640::getFrameSize(void)
{
    return _cam_config.frame_size;
}

void OV5640::setFrameSize(framesize_t size)
{
    _cam_config.frame_size = size;
}

pixformat_t OV5640::getPixelFormat(void)
{
    return _cam_config.pixel_format;
}

void OV5640::setPixelFormat(pixformat_t format)
{
    switch (format)
    {
...
esp_err_t OV5640::init(camera_config_t config)
{
